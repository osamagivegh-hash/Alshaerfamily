...
import { cmsDashboardApi } from '../../utils/adminApi'
import toast from 'react-hot-toast'

/**
 * CMS Backup Manager Component
 * 
 * Features:
 * - List all CMS backups
 * - Create manual backups (Admin/Super Admin)
 * - Restore from backup (Super Admin only)
 * - Delete backups (Super Admin only)
 * - View backup statistics
 */
const CMSBackupManager = () => {
    const { user } = useAdmin()
    ...
    const fetchBackups = useCallback(async () => {
        try {
            ...
            const [backupsRes, statsRes] = await Promise.all([
                cmsDashboardApi.getBackups(20),
                cmsDashboardApi.getStats()
            ])
    ...
    const handleCreateBackup = async () => {
        ...
        if (!isAdmin) {
            toast.error('صلاحيات المدير مطلوبة لإنشاء نسخة احتياطية')
            return
        }

        try {
            setCreating(true)
            const result = await cmsDashboardApi.createBackup()
    ...
    const handleConfirmRestore = async () => {
        ...
        try {
            ...
            const result = await cmsDashboardApi.restoreBackup(confirmingRestore, true)
            toast.success(
                `${result.message}\n` +
    ...
    const handleDeleteBackup = async (backupId) => {
        ...
        try {
            await cmsDashboardApi.deleteBackup(backupId)
    ...
    return (
        <div className="bg-white rounded-lg shadow-lg p-6" dir="rtl">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                        ...
                        النسخ الاحتياطية لنظام إدارة المحتوى
                    </h2>
                    <p className="text-gray-500 text-sm mt-1">
                        إدارة النسخ الاحتياطية للأخبار والمقالات والمحتوى
                    </p>
                </div>
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 text-center">
                        <div className="text-2xl font-bold text-blue-600">{stats.content?.news || 0}</div>
                        <div className="text-xs text-blue-700">أخبار</div>
                    </div>
                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 text-center">
                        <div className="text-2xl font-bold text-purple-600">{stats.content?.articles || 0}</div>
                        <div className="text-xs text-purple-700">مقالات</div>
                    </div>
                    <div className="bg-gradient-to-br from-pink-50 to-pink-100 rounded-lg p-4 text-center">
                        <div className="text-2xl font-bold text-pink-600">{stats.content?.conversations || 0}</div>
                        <div className="text-xs text-pink-700">حوارات</div>
                    </div>
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 text-center">
                        <div className="text-2xl font-bold text-green-600">{stats.backup?.totalBackups || 0}</div>
                        <div className="text-xs text-green-700">نسخ احتياطية</div>
                    </div>
                    <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 text-center">
                        <div className="text-xs text-orange-700 mb-1">آخر نسخة</div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
                        ...
                        <p className="text-gray-600 text-center mb-6 leading-relaxed">
                            ...
                            <br />
                            <strong className="text-red-600">سيتم حذف جميع المحتوى الحالي</strong>
                            <br />
                            (الأخبار، المقالات، الحوارات، الصور، إلخ)
                            <br />
            ...
            {backups.length > 0 && backups[0]?.stats?.collections && (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <h4 className="text-sm font-semibold text-gray-700 mb-3">محتويات آخر نسخة احتياطية:</h4>
                    <div className="flex flex-wrap gap-2">
                        {backups[0].stats.collections.map((col) => (
                            <span key={col.name} className="px-3 py-1 bg-white rounded-full text-xs shadow-sm">
                                {col.name}: <strong>{col.count}</strong>
                            </span>
                        ))}
                    </div>
                </div>
            ...
            {!isAdmin && (
                <div className="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
                    <div className="flex items-center gap-2 text-red-700">
                        ...
                        <p>صلاحيات المدير مطلوبة لإدارة النسخ الاحتياطية لنظام إدارة المحتوى.</p>
                    </div>
                </div>
            )}
...
export default CMSBackupManager
